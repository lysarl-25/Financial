...
<template>
  <div class="card p-5">
    <div class="flex items-center justify-between mb-4">
      <h3 class="font-display text-base font-semibold text-ink-900 dark:text-ink-50">Recent Transactions</h3>
      <RouterLink to="/transactions" class="text-sm font-medium text-ink-600 hover:text-ink-900 dark:text-ink-300 dark:hover:text-white">
        View all
      </RouterLink>
    </div>

    <EmptyState v-if="!transactionStore.recent.length" title="No transactions yet" description="Add your first transaction to see it here." />
    <ul v-else class="divide-y divide-ink-100 dark:divide-ink-800">
      <li v-for="t in transactionStore.recent" :key="t.id" class="flex items-center justify-between py-3">
        <div class="flex items-center gap-3 min-w-0">
          ...
          <div class="min-w-0">
            <p class="text-sm font-medium text-ink-800 dark:text-ink-100 truncate">{{ t.description }}</p>
